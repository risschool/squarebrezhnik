	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#Squarebrezhnik-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#Squarebrezhnik-contact').find('input,textarea').attr({value:''});
				jQuery('#Squarebrezhnik-contact').css({opacity:1});
				jQuery('#Squarebrezhnik-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#Squarebrezhnik-contact').animate({opacity:0.3});
		jQuery('#Squarebrezhnik-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#Squarebrezhnik-contact').find('input,textarea').css('border','none');
